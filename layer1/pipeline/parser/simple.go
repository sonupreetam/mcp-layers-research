...
// isTOCLine checks if a line appears to be from a Table of Contents
//
//nolint:unused // Reserved for enhanced TOC detection
func isTOCLine(line string) bool {
	// TOC lines typically have:
	// 1. Dotted leaders (multiple consecutive dots)
	// 2. End with page numbers
	// 3. Multiple entries concatenated together
	
	dotCount := strings.Count(line, ".")
	
	// If there are many dots (like in TOC leaders), it's likely a TOC line
	if dotCount > 20 {
		return true
	}
	
	// Check for dot-leader pattern: text....number
	tocPattern := regexp.MustCompile(`\.{4,}\s*\d+`)
	return tocPattern.MatchString(line)
}
